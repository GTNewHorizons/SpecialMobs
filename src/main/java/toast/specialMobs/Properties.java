...
/**
 * This helper class automatically creates, stores, and retrieves properties. Supported data types: String, boolean,
 * int, double
 *
 * Any property can be retrieved as an Object or String. Any non-String property can also be retrieved as any other
 * non-String property. Retrieving a number as a boolean will produce a randomized output depending on the value.
 */
public abstract class Properties {
    ...
    /// Property array, matched up to MONSTER_KEY[], vanilla monster species will only exist if the same index here is
    /// true.
    private static boolean[] monsterVanilla = new boolean[_SpecialMobs.MONSTER_KEY.length];
    /// Property array, matched up to MONSTER_KEY[], special monster species will only spawn if the same index here is
    /// true.
    private static boolean[] monsterSpawn = new boolean[_SpecialMobs.MONSTER_KEY.length];
    /// 2D property array, matched up to MONSTER_TYPES[][1], weights for each monster subspecies. monsterWeights[][0]
    /// refers to the vanilla weight.
    ...
    public static void init(Configuration config) {
        ...
        Properties.add(
                config,
                Properties.LAVAMONSTER_GENERAL,
                "lavamonster_animated_texture",
                true,
        ...
        Properties.add(
                ...
                "spawn_eggs",
                false,
                "If true, the game will attempt to make a spawn egg for each special mob. Be warned, this will eat up "
                        + mobsAdded
                        + " global entity ids. Default is false.");
        ...
        Properties.initializeArrays(
                config,
                "monster",
                _SpecialMobs.MONSTER_KEY,
                _SpecialMobs.MONSTER_TYPES,
                Properties.monsterVanilla,
                Properties.monsterSpawn,
                Properties.monsterWeights);
    ...
    private static void loadIntSet(Configuration config, String category, String field, String defaultValue,
            String comment, HashSet<Integer> intSet) {
        intSet.clear();
    ...
    private static void initializeArrays(Configuration config, String category, String[] key, String[][] types,
            boolean[] keyVanilla, boolean[] keyValues, int[][] typeWeights) {
        String pluralKey;
        String MOB_CATEGORY;
        for (int i = 0; i < key.length; i++) {
            ...
            keyVanilla[i] = config
                    .get(
                            ...
                            "_allow_vanilla",
                            false,
                            "If this false, vanilla " + pluralKey
                                    + " will be replaced with similar Special Mobs versions if not \"special\".")
                    .getBoolean(true);
            keyValues[i] = config.get(
                    MOB_CATEGORY,
                    "_special_" + pluralKey,
                    true,
                    "Set this to false to disable all special " + pluralKey + ".").getBoolean(true);
            ...
            config.addCustomCategoryComment(
                    MOB_CATEGORY,
                    "Weighted chances for each mob subtype when a vanilla " + _SpecialMobs.localizeName(key[i])
                            + " is spawned.");
        }
    }
    ...
    public static void add(Configuration config, String category, String field, int defaultValue) {
        Properties.map.put(
                category + "@" + field,
                Integer.valueOf(config.get(category, field, defaultValue).getInt(defaultValue)));
    }

    public static void add(Configuration config, String category, String field, int defaultValue, String comment) {
        Properties.map.put(
                category + "@" + field,
                Integer.valueOf(config.get(category, field, defaultValue, comment).getInt(defaultValue)));
    }

    public static void add(Configuration config, String category, String field, int defaultValue, int minValue,
            int maxValue) {
        Properties.map.put(
                category + "@" + field,
                Integer.valueOf(
                        Math.max(
                                minValue,
                                Math.min(maxValue, config.get(category, field, defaultValue).getInt(defaultValue)))));
    }
        ...
        Properties.map.put(
                ...
                Integer.valueOf(
                        Math.max(
                                ...
                                Math.min(
                                        maxValue,
                                        config.get(category, field, defaultValue, comment).getInt(defaultValue)))));
    }

    public static void add(Configuration config, String category, String field, boolean defaultValue) {
        Properties.map.put(
                category + "@" + field,
                Boolean.valueOf(config.get(category, field, defaultValue).getBoolean(defaultValue)));
    }

    public static void add(Configuration config, String category, String field, boolean defaultValue, String comment) {
        Properties.map.put(
                category + "@" + field,
                Boolean.valueOf(config.get(category, field, defaultValue, comment).getBoolean(defaultValue)));
    }

    public static void add(Configuration config, String category, String field, double defaultValue) {
        Properties.map.put(
                category + "@" + field,
                Double.valueOf(config.get(category, field, defaultValue).getDouble(defaultValue)));
    }

    public static void add(Configuration config, String category, String field, double defaultValue, String comment) {
        Properties.map.put(
                category + "@" + field,
                Double.valueOf(config.get(category, field, defaultValue, comment).getDouble(defaultValue)));
    }

    public static void add(Configuration config, String category, String field, double defaultValue, double minValue,
            double maxValue, String comment) {
        Properties.map.put(
                category + "@" + field,
                Double.valueOf(
                        Math.max(
                                ...
                                Math.min(
                                        maxValue,
                                        config.get(category, field, defaultValue, comment).getDouble(defaultValue)))));
    }
    ...
    public static int getInt(String category, String field) {
        ...
        if (property instanceof Boolean) return ((Boolean) property).booleanValue() ? 1 : 0;
        Properties.debugException(
                "Tried to get int for invalid property! @" + property == null ? "(null)"
