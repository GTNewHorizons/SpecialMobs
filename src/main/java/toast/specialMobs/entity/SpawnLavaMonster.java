...
public class SpawnLavaMonster {
    ...
    public static final int SPAWN_FREQUENCY = Properties
            .getInt(Properties.LAVAMONSTER_SPAWNING, "lavamonster_spawn_frequency");
    ...
    public static final double SPAWN_CHANCE = Properties
            .getDouble(Properties.LAVAMONSTER_SPAWNING, "lavamonster_spawn_chance");

    public static final boolean DEPTH_HAZARD = Properties
            .getBoolean(Properties.LAVAMONSTER_SPAWNING, "lavamonster_depth_hazard");
    public static final boolean SHALLOW_LAVA = Properties
            .getBoolean(Properties.LAVAMONSTER_SPAWNING, "lavamonster_shallow_lava");
    public static final boolean FLOWING_LAVA = Properties
            .getBoolean(Properties.LAVAMONSTER_SPAWNING, "lavamonster_flowing_lava");
    public static final boolean NETHER_SPAWN = Properties
            .getBoolean(Properties.LAVAMONSTER_SPAWNING, "lavamonster_Nether_spawn");

    /// Counter to the next spawn attempt.
    private int spawnTime = 0;
    /// A map of eligible spawning chunks.
    private HashMap<ChunkCoordIntPair, Boolean> eligibleChunksForSpawning = new HashMap<ChunkCoordIntPair, Boolean>(); // location
                                                                                                                       // of
                                                                                                                       // chunk
                                                                                                                       // to
                                                                                                                       // spawn
    ...
    public static boolean canLavaMonsterSpawnAtLocation(World world, int x, int y, int z) {
        return (!SpawnLavaMonster.DEPTH_HAZARD || y <= 16 || world.provider.isHellWorld)
                && world.getBlock(x, y, z).getMaterial() == Material.lava
                && (world.getBlock(x, y + 1, z).getMaterial() == Material.lava
                        || SpawnLavaMonster.SHALLOW_LAVA && !world.isBlockNormalCubeDefault(x, y + 1, z, true))
                && (SpawnLavaMonster.FLOWING_LAVA || world.getBlockMetadata(x, y, z) == 0
    ...
    private int performSpawning(WorldServer world) {
        ...
        if (!SpawnLavaMonster.NETHER_SPAWN && world.provider.isHellWorld
                || (++this.spawnTime < SpawnLavaMonster.SPAWN_FREQUENCY))
            return 0;

        // Reset spawnTimer to 0
        this.spawnTime = 0;
        this.eligibleChunksForSpawning.clear();
        for (EntityPlayer player : (List<EntityPlayer>) world.playerEntities) { // This cycles across *all* player
                                                                                // entities
            ...
            for (int x = -spawnRange; x <= spawnRange; x++) {
                for (int z = -spawnRange; z <= spawnRange; z++) {
                    ...
                    // And if a chunk is already marked, it then clears it. This prevents spawns far away from players
                    // that won't get seen etc anyways.
                    ...
                    if (!isEdge) {
                        this.eligibleChunksForSpawning.put(chunkCoord, Boolean.valueOf(false)); // If a chunk is already
                                                                                                // in the list, override
                                                                                                // it as false, it's not
                                                                                                // an edge chunk any
                                                                                                // more
                    } else if (!this.eligibleChunksForSpawning.containsKey(chunkCoord)) { // isEdge is true, and the
                                                                                          // chunk is not already in the
                                                                                          // HashMap
                        this.eligibleChunksForSpawning.put(chunkCoord, Boolean.valueOf(true)); // OK to place a lava
                                                                                               // monster in this spot
                    }
                }
            }
        }
        ...
        int adjustTotalChunks = (SPAWN_RANGE * 2 + 1) * (SPAWN_RANGE * 2 + 1); // Above for loop is over *all* player
                                                                               // entities. To get the correct number of
                                                                               // maximum lava monsters, we have to
                                                                               // adjust
                                                                               // by how many lava monsters are allowed
                                                                               // across all possible spawning chunks.
                                                                               // SPAWN_MAX per player
        // Since it only checks here for SPAWN_MAX, it's possible there could be more than SPAWN_MAX lava monsters in
        // the world.
        ...
        if (countInWorld < maxAllowedInWorld) {
            ...
            chunkIterator: for (ChunkCoordIntPair chunkCoord : chunks) {
                ...
                if (!this.eligibleChunksForSpawning.get(chunkCoord).booleanValue()) { // Don't spawn lava monsters on
                                                                                      // the chunks furthest from the
                                                                                      // player. Keeps them a little
                                                                                      // close and keeps a buffer
                                                                                      // distance
                    ChunkPosition chunkPos = this
                            .getRandomSpawningPointInChunk(world, chunkCoord.chunkXPos, chunkCoord.chunkZPos);
                    ...
                    for (int groupSpawnAttempt = 3; groupSpawnAttempt-- > 0;) {
                        ...
                        for (int spawnAttempt = 4; spawnAttempt-- > 0;) {
                            ...
                            if (SpawnLavaMonster.canLavaMonsterSpawnAtLocation(world, X, Y, Z)) {
                                ...
                                float posY = Y;
                                float posZ = Z + 0.5F;
                                if (world.getClosestPlayer(posX, posY, posZ, 24.0) == null) { // No player closer than
                                                                                              // 24 blocks
                                    ...
                                    if (spawnDist >= 576.0F) { // Make sure we aren't filling the spawn chunks with lava
                                                               // monsters
                                        EntityLavaMonster lavaMonster = new EntityLavaMonster(world);
                                        lavaMonster.setLocationAndAngles(
                                                posX,
                                                posY,
                                                posZ,
                                        ...
                                        Result canSpawn = ForgeEventFactory
                                                .canEntitySpawn(lavaMonster, world, posX, posY, posZ);
                                        if (canSpawn == Result.ALLOW
                                                || canSpawn == Result.DEFAULT && lavaMonster.getCanSpawnHere()) {
                                            numberSpawned++;
                                            _SpecialMobs.debugConsole(
                                                    "Mob Spawned at X " + posX + "  Y " + posY + "  Z " + posZ);
                                            world.spawnEntityInWorld(lavaMonster);
                                            if (!ForgeEventFactory
                                                    .doSpecialSpawn(lavaMonster, world, posX, posY, posZ)) {
