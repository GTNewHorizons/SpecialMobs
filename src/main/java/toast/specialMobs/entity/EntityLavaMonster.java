...
public class EntityLavaMonster extends EntityMob implements ISpecialMob {
    ...
    public static final boolean START_FIRES = Properties
            .getBoolean(Properties.LAVAMONSTER_GENERAL, "lavamonster_fires");
    public static final boolean BASIC_LOOT = Properties
            .getBoolean(Properties.LAVAMONSTER_GENERAL, "lavamonster_basic_loot");
    public static final int HEAL_TIME = Properties.getInt(Properties.LAVAMONSTER_GENERAL, "lavamonster_heal_time");
    public static final double HEAL_PERCENT = Properties
            .getDouble(Properties.LAVAMONSTER_GENERAL, "lavamonster_heal_percent");
    ...
    public void onLivingUpdate() {
        ...
        if (this.rand.nextInt(100) == 0) {
            this.worldObj.spawnParticle(
                    ...
                    this.posY + this.rand.nextDouble() * this.height + this.height / 2.0,
        ...
        // On average, every HEAL_TIME (multiplied by 2 because average) heal the lava monster if it is standing in
        // lava. This way players have to lure it out to do more damage
        if ((this.rand.nextInt(HEAL_TIME * 2) == 0)
                && (this.worldObj.getBlock((int) Math.round(posX), (int) Math.round(posY), (int) Math.round(posZ))
        ...
        // 2 extra age ticks is added in bright areas, but since this spawns in lava, we want to negate those 2 extra
        // ticks.
    ...
    public void attackEntityWithFireball(EntityLivingBase target) {
        EntitySmallFireball fireball = new EntitySmallFireball(
                ...
                target.boundingBox.minY + target.height / 2.0F - this.posY - this.height / 2.0F,
        ...
        if ((superRare == 0) || BASIC_LOOT) {
            ...
            switch (this.rand.nextInt(4)) {
                case 0:
                    ...
                    drop.setStackDisplayName("\u00a7cFirefighter Boots"); // In honor of the brave men and women who run
                                                                          // into danger, not away
    ...
    public void updateTexture() {
        if (!EntityLavaMonster.ANIMATE_TEXTURE || ++this.textureTicks < 2) return;
        this.textureTicks = 0;
    ...
    public boolean getCanSpawnHere() {
        return this.rand.nextDouble() < EntityLavaMonster.SPAWN_CHANCE
