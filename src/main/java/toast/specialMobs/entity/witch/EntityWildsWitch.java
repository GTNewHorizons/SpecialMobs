...
public class EntityWildsWitch extends Entity_SpecialWitch {
    ...
    public void tryDrinkPotion() {
        if (this.potionThrowDelay <= 0) {
            EntityLivingBase riding = this.ridingEntity instanceof EntityLivingBase
            ...
            if (riding != null && riding.isBurning() && !riding.isPotionActive(Potion.fireResistance)) {
            ...
            } else if (this.rand.nextFloat() < 0.2F && riding != null
                    && riding.isInsideOfMaterial(Material.water)
                    && !riding.isPotionActive(Potion.waterBreathing)
                    || this.rand.nextFloat() < 0.15F && this.isInsideOfMaterial(Material.water)
                            && !this.isPotionActive(Potion.waterBreathing)) {
                                if (riding != null && riding.isInsideOfMaterial(Material.water)
                                        && !riding.isPotionActive(Potion.waterBreathing)) {
                ...
                if (this.rand.nextFloat() < 0.1F && riding != null && riding.getHealth() < riding.getMaxHealth()
                    ...
                    if (this.spiderCount > 0 && this.ridingEntity == null) {
