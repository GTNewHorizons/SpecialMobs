...
public abstract class RandomHelper {
    ...
    public static EntityLiving nextMonster(int key, World world) {
        return RandomHelper.nextEntity(
                ...
                world,
                "monster",
                _SpecialMobs.MONSTER_KEY[key],
                _SpecialMobs.MONSTER_TYPES[key],
    ...
    public static EntityLiving nextEntity(int key, World world, String category, String mobKey, String[] types,
            int[] weights, int totalWeight) {
        int choice = _SpecialMobs.random.nextInt(totalWeight);
        for (int i = weights.length; i-- > 0;) {
            choice -= weights[i];
            if (choice < 0) {
                if (i == 0) return Properties.monsterVanilla()[key] ? null
                        : (EntityLiving) EntityList.createEntityByName("SpecialMobs.Special" + mobKey, world);
